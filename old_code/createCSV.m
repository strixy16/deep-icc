...
%   Sept 3, 2021 - this is broken, the labels are wrong at the bottom of
%   the generated file
    ...
    [label_num,label_txt,~] = xlsread(options.Labels);

    % get just the patient ids from label spreadsheet
    pats_with_labels = label_txt(2:end,1);
    % match up these names with the unique patient ids that we have slices
    % for
    % pwl = patients with labels
    [pwl_fnames, ut_idx, pwl_idx] = intersect(ut, pats_with_labels);
    asc_num = (1:length(ut))';
    ...
    for i=1:length(nl_patient)
        ...
        % nl_pat_slice_idx is patient indices with no labels
    ...
    % This is redundant, rl_patient and label_num are equal
    rl_patient = label_num(pwl_idx,:);
    % withlabel_slices
    lbl_4_slices = [];
   
    % THIS IS WHERE THE PROBLEM IS 
    for idx=1:length(rl_patient)
        recurrence_lbl_for_slices = rl_patient(idx,:)' .* ones(size(label_num,2),num_slices_per_pat(ut_idx(idx)));
%         time_lbl_for_slices = num(ut_idx(idx),2) * ones(1,num_slices_per_pat(ut_idx(idx)));
