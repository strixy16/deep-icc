...
def valid_epoch(model, device, dataloader, criterion):
    ...
    return val_loss, val_all_cind, df_all_pred
...
def kfold_train(data_info_path, data_img_path, out_dir_path, k=None, seed=16):
    ...
    for fold, (train_idx, val_idx) in enumerate(group_kfold.split(hdfs_dataset.img_fname, hdfs_dataset.time_label, groups)):
        ...
        for epoch in range(args.EPOCHS):
            ...
            print("Epoch:{}/{} AVG Training Loss:{:.3f} AVG Validation Loss:{:.3f} "
                  "AVG Training C-index:{:.2f}  Validation C-index:{:.2f} ".format(epoch + 1,
                                                                                         args.EPOCHS,
                                                                                         train_loss,
                                                                                         valid_loss,
                                                                                         train_cind,
    ...
    print("C-index for predictions from all folds: {:.3f}".format(val_all_cind))
    ...
    for f in range(1,args.K+1):
        ...
        # TODO: instead of averaging across all epochs, just print the final prediction from each fold and then print the validation c-index and loss for the entire validation set 
