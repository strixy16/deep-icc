...
def train_ct_and_gene(config, checkpoint_dir=None, data_dir=None):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    train_loader, _ = load_chol_tumor_w_gene(data_dir,
                                             imdim=args.imdim,
...
def main(num_samples=10, gpus_per_trial=1):
    ...
    reporter = CLIReporter(
        ...
        metric_columns=["loss", "c_index", "training_iteration"])
    ...
    print("Best trial final c-index: {}".format(best_trial.last_result["c_index"]))
